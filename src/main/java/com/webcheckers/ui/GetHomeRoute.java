...
public class GetHomeRoute implements Route {
    ...
    public Object handle(Request request, Response response) {
        ...
        String delGame = request.queryParams("delGame");
        if (delGame != null) {
            ...
            if (currentGame != null) {
                response.redirect("/");
                currentGame.decrementPlayersInGame();
                if (currentGame.getPlayersInGame() == 0 || currentUser.equals(currentGame.getWinner())) {
                    gameCenter.removeGame(currentGame.getRedPlayer(), currentGame.getWhitePlayer(), currentGame.getGameID());
                }
            }
        }
